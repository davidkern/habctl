//! Modified tide::fs::serve_dir to add index support
...
pub trait ServeDirWithIndex {
    fn serve_dir_with_index(&mut self, dir: impl AsRef<Path>) -> io::Result<()>;
}

impl<'a, State: Clone + Send + Sync + 'static> ServeDirWithIndex for tide::Route<'a, State> {
    fn serve_dir_with_index(&mut self, dir: impl AsRef<Path>) -> io::Result<()> {
        // Verify path exists, return error if it doesn't.
        let dir = dir.as_ref().to_owned().canonicalize()?;
        let prefix = self.path().to_string();
        let endpoint = ServeDir::new(prefix, dir);
